...
<div class="panel mt-3">
    <div class="panel-header">
        <span class="panel-title">
            <i class="bi bi-shield-lock me-2 {{ $indicador['total_horas'] >= 216 ? 'text-danger' : 'text-secondary' }}"></i>
        ...
        @elseif($indicador['total_horas'] >= 216)
            <span class="badge bg-danger">
                <i class="bi bi-x-circle me-1"></i>Límite alcanzado
            </span>
        @else
            <span class="badge bg-success-subtle text-success">
                {{ number_format($indicador['total_horas'], 0) }} / 216h
            </span>
        @endif
    </div>
    <div class="panel-body">
        {{-- Barra de progreso --}}
        @php $pct = min(100, ($indicador['total_horas'] / 216) * 100); @endphp
        ...
        <p class="small text-muted mb-3">
            {{ number_format($indicador['total_horas'], 0) }} h registradas de 216h permitidas.
            @if($autorizacionHoras)
        ...
        @else
            ...
            <form method="POST" action="{{ route('medicos.autorizar-horas', $medico) }}">
                ...
                <button class="btn btn-sm btn-warning fw-semibold">
                    <i class="bi bi-unlock me-1"></i>Autorizar superar 216h este mes
                </button>
            </form>
        @endif
        @endif
